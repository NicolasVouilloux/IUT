...
Image::Image(const vector<vector<int>> &R, const vector<vector<int>> &B,
...
// operateur pour comparer deux images
...
Image Image::contrasteUp () {
  ...
  for (int i = 0; i < _longueur; ++i) {
    ...
    for (int j = 0; j < _largeur; ++j)
    {
      ...
      if (I4._red[i][j] > 255) {I4._red[i][j] = 255;}
      if (I4._red[i][j] < 0) {I4._red[i][j] = 0;}
      I4._blue[i].push_back((_blue[i][j] - 128)*niv+128);
      if (I4._blue[i][j] > 255) {I4._blue[i][j] = 255;}
      if (I4._blue[i][j] < 0) {I4._blue[i][j] = 0;}
      I4._green[i].push_back((_green[i][j] - 128)*niv+128);
      if (I4._green[i][j] > 255) {I4._green[i][j]=255;}
      if (I4._green[i][j] < 0) {I4._green[i][j] = 0;}
    }
    }
  return I4;
  }
