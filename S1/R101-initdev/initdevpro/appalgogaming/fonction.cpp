...
void Image::menu() {
  ...
  do {
    ...
    if (rep == 16) {
      ...
      int nb=0;
      cin>>nb,
      (RognerB(nb)).Fichierppm(of);
    }
    if (rep == 17) {
      cout <<"Vous avez choisi de retourner l'image sur l'axe horizontale"<<endl;
      (retournementV()).Fichierppm(of);
    }
    if (rep == 18) {
      cout <<"Vous avez choisi de retourner l'image sur l'axe verticale"<<endl;
...
Image Image::RognerB(int &nb) {
  Image c;
  c._longueur = (_red.size() - 1);
  c._largeur = (_red[0].size());

  for (int i = 0; i < _longueur; ++i) {
    c._red.push_back(vector<int>{});
    c._blue.push_back(vector<int>{});
    c._green.push_back(vector<int>{});
    for (int j = 0; j < _largeur; ++j) {
      c._red[i].push_back(_red[i][j]);
      c._blue[i].push_back(_blue[i][j]);
      c._green[i].push_back(_green[i][j]);
    }
  }
  return c;
}
...
Image Image::RognerH (int &nb)
{
    ...
    for (int i = 1; i <= c._longueur ; ++i) {
    c._red.push_back(vector<int>{});
    c._blue.push_back(vector<int>{});
    c._green.push_back(vector<int>{});
    for (int j = 0; j < _largeur; ++j) {
      c._red.back().push_back(_red[i][j]);
      c._blue.back().push_back(_blue[i][j]);
      c._green.back().push_back(_green[i][j]);
